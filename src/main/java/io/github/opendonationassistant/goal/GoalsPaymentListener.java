...
public class GoalsPaymentListener {
  ...
  public void listen(CompletedPaymentNotification payment) {
    ...
    Optional<Goal> oldGoal = Optional
    ...
    oldGoal.map(goal -> goal.getWidgetId())
      .ifPresent(widgetId -> {
        List<Goal> goalList = goalFactory.findFor(payment.getRecipientId(), widgetId);
        ...
        goals.setValue(
          goalList.stream()
            ...
            .reduce(
              new ArrayList<>(),
              (list, goal) -> { list.add(goal); return list;},
